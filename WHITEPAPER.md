...
Here we present giot, a novel blockchain network architecture that addresses all
of these problems.  giot is a network connecting many independent blockchains, called
...
well suited for scaling public proof-of-stake blockchains.  Blockchains with other consensus models, including proof-of-work blockchains like Ethereum and Bitcoin can be connected to the giot network using adapter zones.

The first zone on giot is called the giot Hub. The giot Hub is a
multi-asset proof-of-stake cryptocurrency with a simple governance mechanism
which enables the network to adapt and upgrade.  In addition, the giot Hub can be
extended by connecting other zones.

The hub and zones of the giot network communicate with each other via an
...
inter-zone token transfers go through the giot Hub, which keeps track of the
total amount of tokens held by each zone.  The hub isolates each zone from the
failure of other zones.  Because anyone can connect a new zone to the giot Hub,
zones allow for future-compatibility with new blockchain innovations.

With giot interoperability between blockchains can be achieved. The potential of an internet of value, where assets are issued and controlled by different sets of validators, yet can be moved and exchanged seamlessly between blockchains without relying on trusted third parties can be realized.
...
in a proof-of-stake application such as the giot Hub, the voting power may be
...
![Figure of Tendermint throughput performance](https://raw.githubusercontent.com/gnuclear/give-whitepaper/master/images/tendermint_throughput_blocksize.png)
...
The first blockchain in this network will be the giot Hub.  The giot Hub
connects to many other blockchains (or _zones_) via a novel inter-blockchain
communication protocol.  The giot Hub tracks numerous token types and keeps
...
The giot Hub is the first public blockchain in the giot Network, powered by
...
Validators in giot have a similar role to Bitcoin miners, but instead use
...
staking tokens (called "gives") to any validator to earn a portion of block fees
and give rewards, but they incur the risk of getting punished (slashed) if the
...
Validators and delegators on the giot Hub can vote on proposals that can
...
constitution that govern the policies of the giot Hub.  The constitution
...
For example, the giot Hub could have a constitution that enforces immutability
at the Hub (no roll-backs, save for bugs of the giot Hub node implementation),
while each zone can set their own policies regarding roll-backs.

By enabling interoperability among differing policy zones, the giot network
gives its users ultimate freedom and potential for permissionless
experimentation.

## The Hub and Zones ###########################################################

Here we describe a novel model of decentralization and scalability.  giot is a
network of many blockchains powered by Tendermint.  While existing proposals aim
to create a "single blockchain" with total global transaction ordering, giot
permits many blockchains to run concurrently with one another while retaining
interoperability.

At the basis, the giot Hub manages many independent blockchains called "zones"
...
The giot Hub is a blockchain that hosts a multi-asset distributed ledger,
...
Since the giot Hub acts as the central ledger for the whole
...
A giot zone is an independent blockchain that exchanges IBC messages with the
...
gives of the giot Hub may be staked by validators of a zone connected to the
Hub.  While double-spend attacks on these zones would result in the slashing of
gives with Tendermint's fork-accountability, a zone where >⅔ of the voting power
are Byzantine can commit invalid state.  The giot Hub does not verify or
execute transactions committed on other zones, so it is the responsibility of
users to send tokens to zones that they trust.  In the future, the giot Hub's
...
based on something called "giveic cross-chain" (AXC) transactions.  With an AXC
...
giveically).  For example, two users can trade bitcoins for ether (or any two
...
prioritizing fast finality without sacrificing consistency, zones in giot can
...
giot is the distributed exchange (aka the giot DEX).  The transaction
...
without both parties having to be online.  And with Tendermint, the giot hub,
...
giot hub and zone; both must keep up with the latest blocks of the
other in order to verify proofs that tokens have moved from one to the other.  A
"bridge-zone" on the giot network keeps up with the Hub as well as the
...
#### Sending Tokens to the giot Hub
...
giot Hub.  On the Ethereum side (the origin), a bridge-contract would allow
...
may be identical to the giot Hub's validator-set.
...
identical to the giot Hub validator-set.

#### Withdrawing Tokens from giot Hub
...
system open as a future giot improvement proposal, to be passed by the giot
...
giot Hub and IBC packet mechanics does not allow for arbitrary contract logic
...
giot zones run arbitrary application logic, which is defined at the beginning of the
zone's life and can potentially be updated over time by governance. Such flexibility
allows giot zones to act as bridges to other cryptocurrencies such as Ethereum or
...
outputs, where each input can be any token type, enabling giot to serve
...
public giot network without sacrificing control over the underlying service.
Thus, giot may offer the best of both worlds for organizations looking to
...
altogether. The giot architecture can help mitigate this problem by using a global
...
while sharing a common hub (e.g. the giot Hub), enabling municipal activity to
...
In giot, we can take this concept and extend it further. Each
name-registration zone in giot can have an associated top-level-domain
...
While the giot Hub is a multi-asset distributed ledger, there is a special
native token called the _give_.  gives are the only staking token of the giot
Hub.  gives are a license for the holder to vote, validate, or delegate to other
validators.  Like Ethereum's ether, gives can also be used to pay for
transaction fees to mitigate spam.  Additional inflationary gives and block
transaction fees are rewarded to validators and delegators who delegate to
validators.

The `BurngiveTx` transaction can be used to recover any proportionate amount of
tokens from the reserve pool.

#### Fundraiser

The initial distribution of give tokens and validators on Genesis will go to the
donors of the giot Fundraiser (75%), lead donors (5%), giot Network
Foundation (10%), and ALL IN BITS, Inc (10%).  From genesis onward, 1/3 of the
total amount of gives will be rewarded to bonded validators and delegators
every year.

See the [giot Plan](https://github.com/giot/giot/blob/master/PLAN.md)
...
give holders who are not already can become validators by signing and
submitting a `BondTx` transaction.  The amount of gives provided as collateral
...
of gives is greater than the amount of effective gives held by the smallest
validator, where effective gives include delegated gives.  When a new validator
replaces an existing validator in such a way, the existing validator becomes
inactive and all the gives and delegated gives enter the unbonding state.
...
bonded gives as well its proportionate share of tokens in the reserve pool --
...
In situations where the giot Hub halts due to a ≥⅓ coalition of voting power
...
giot Hub validators can accept any token type or combination of types as fees
...
their bonded gives, every `ValidatorPayoutPeriod` (DEFAULT 1 hour).
...
the giot network. These funds can also be distributed in accordance with the
decisions made by the governance system.

give holders who delegate their voting power to other validators pay a
commission to the delegated validator.  The commission can be set by each
validator.

### Incentivizing Hackers

The security of the giot Hub is a function of the security of the underlying
validators and the choice of delegation by delegators.  In order to encourage
the discovery and early reporting of found vulnerabilities, the giot Hub
...
will become inactive, `HackPunishmentRatio` (default 5%) of everyone's gives
will get slashed, and `HackRewardRatio` (default 5%) of everyone's gives will
get rewarded to the hacker's bounty address.  The validator must recover the
remaining gives by using their backup key.

In order to prevent this feature from being abused to transfer unvested gives,
the portion of vested vs unvested gives of validators and delegators before and
after the `ReportHackTx` will remain the same, and the hacker bounty will
include some unvested gives, if any.

### Governance Specification ###################################################

The giot Hub is operated by a distributed organization that requires a well-defined
...
may be overridden manually.  Unbonded gives get no vote.

Each proposal requires a deposit of `MinimumProposalDeposit` tokens, which may
be a combination of one or more tokens including gives.  For each proposal, the
...
reserve pool, except any gives which are burned.
...
punished by losing `VetoPenaltygives` (DEFAULT 0.1%) of its gives.
...
gives can be inflated and reserve pool funds spent with the passing of a `BountyProposal`.
...
See [the Plan](https://github.com/giot/giot/blob/master/PLAN.md).
...
giveic clocks.  It was not until the late 90s that Practical Byzantine Fault
...
view-changes.  In giot and many cryptocurrencies, there is no need to allow
...
hasn't committed in a giot zone, then it doesn't help to use bandwidth sharing
...
token called the _give_ is issued that represent claims to future portions of
...
payments across disparate ledger types, and extends the giveic transaction
mechanism to include not only hash-locks, but also a quorum of notaries (called
the giveic Transport Protocol).  The latter mechanism for enforcing giveicity
in inter-ledger transactions is similar to Tendermint's light-client SPV
mechanism, so an illustration of the distinction between ILP and giot/IBC is
...
3. The most striking difference is that ILP's connectors are not responsible or
   keeping authoritative state about payments, whereas in giot, the validators
...
analog to ILP's connector in giot is a persistent and maximally secure
blockchain ledger, the giot Hub.
...
blockchain. (Two-way pegging is equivalent to bridging. In giot we say
...
allow for experimentation in new features on the sidechain.  As in the giot
...
natively support a variety of tokens and inter-zone network topology as giot
does. That said, the core mechanism of the two-way peg is in principle the same
as that employed by the giot network.
...
##### giot vs Ethereum 2.0 Mauve

giot and Ethereum 2.0 Mauve [\[22\]][22] have different design goals.

* giot is specifically about tokens.  Mauve is about scaling general computation.
* giot is not bound to the EVM, so even different VMs can interoperate.
* giot lets the zone creator determine who validates the zone.
* Anyone can start a new zone in giot (unless governance decides otherwise).
...
cross-chain giveic swaps.  By opening payment channels with many parties,
...
another.  The main benefit of the giot network described here is to enable
...
acknowledgement](https://raw.githubusercontent.com/gnuclear/give-whitepaper/master/msc/ibc_with_ack.png)
...
timeout](https://raw.githubusercontent.com/gnuclear/give-whitepaper/master/msc/ibc_with_ack_timeout.png)

### Merkle Tree & Proof Specification

There are two types of Merkle trees supported in the Tendermint/giot
...
In the canonical implementation, transactions are streamed to the giot hub
application via the ABCI interface.

The giot Hub will accept a number of primary transaction types, including
`SendTx`, `BondTx`, `UnbondTx`, `ReportHackTx`, `SlashTx`, `BurngiveTx`,
...
- `Status (enum)`: Can be one of `AckPending`, `AckSent`, `AckReceived`,
  `NoAck`, or `Timeout`
- `Type (string)`: The types are application-dependent.  giot reserves the
  "coin" packet type
...
providing support for our work with Tendermint and giot.
