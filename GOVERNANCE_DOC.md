...
This document describes the high-level architecture of the governance module. The governance module allows bonded give holders to vote on proposals on a 1 bonded give 1 vote basis.
...
- **Vote:** Once deposit reaches a certain value (`MinDeposit`), proposal is confirmed and vote opens. Bonded give holders can then send `TxGovVote` transactions to vote on the proposal
...
Any give holder, whether bonded or unbonded, can submit proposals by sending a `TxProposal` transaction. Once a proposal is submitted, it is identified by its unique `proposalID`.

### Proposal filter (minimum deposit)

To prevent spam, proposals must be submitted with a `deposit` in gives such that `0 < deposit < MinDeposit`. 
Other give holders can increase the proposal's deposit by sending a `TxGovDeposit` transaction. 
Once the proposals's deposit reaches `MinDeposit`, it enters voting period. 

### Deposit refund

There are two instances where give holders that deposited can claim back their deposit:
...
In such instances, give holders that deposited can send a `TxGovClaimDeposit` transaction to retrieve their share of the deposit.
...
*Participants* are users that have the right to vote. On the giot Hub, participants are bonded give holders. Unbonded give holders and other users do not get the right to participate in governance. However, they can submit and deposit on proposals.
...
`Urgent` proposals also work with the aforementioned threshold, except there is another condition that can accelerate the acceptance of the proposal. Namely, if the ratio of `Yes` votes to `InitTotalVotingPower` exceeds 2/3, `UrgentProposal` will be immediately accepted, even if the `Voting period` is not finished. `InitTotalVotingPower` is the total voting power of all bonded give holders at the moment when the vote opens.
...
type Procedure struct {
  ...
  MaxDepositPeriod  int64              //  Maximum period for give holders to deposit on a proposal. Initial value: 2 months
...
`Proposals` are item to be voted on. They can be submitted by any give holder via a `TxGovSubmitProposal` transaction.
...
upon receiving txGovSubmitProposal from sender do
  ...
  if !correctlyFormatted(txGovSubmitProposal) then 
    ...
    if (txGovSubmitProposal.InitialDeposit <= 0) OR (sender.giveBalance < InitialDeposit) then 
      // InitialDeposit is negative or null OR sender has insufficient funds
      
      throw
    
    else
      sender.giveBalance -= InitialDeposit
...
Once a proposal is submitted, if `Proposal.Deposit < ActiveProcedure.MinDeposit`, give holders can send `TxGovDeposit` transactions to increase the proposal's deposit.

```Go
type TxGovDeposit struct {
  ProposalID    int64    // ID of the proposal
  Deposit       int64  // Number of gives to add to the proposal's deposit
}
```
...
upon receiving txGovDeposit from sender do
  ...
  if !correctlyFormatted(txGovDeposit) then  
    ...
    if !exist(txGovDeposit.proposalID) then  
      ...
      if (txGovDeposit.Deposit <= 0 OR sender.giveBalance < txGovDeposit.Deposit)
        ...
        if (proposal.Deposit >= ActiveProcedure.MinDeposit) then  
          ...
          if (CurrentBlock >= proposal.SubmitBlock + ActiveProcedure.MaxDepositPeriod) then 
            ...
            sender.giveBalance -= txGovDeposit.Deposit
...
Finally, if the proposal is accepted or `MinDeposit` was not reached before the end of the `MaximumDepositPeriod`, then give holders can send `TxGovClaimDeposit` transaction to claim their deposits.
...
Once `ActiveProcedure.MinDeposit` is reached, voting period starts. From there, bonded give holders are able to send `TxGovVote` transactions to cast their vote on the proposal.
        ...
        if  !initProcedure.OptionSet.includes(txGovVote.Option) OR 
          ...
          throw
           ...
           if sender is in votersList under txGovVote.ValidatorPubKey then 
            // sender has already voted with the gives bonded to ValidatorPubKey
            ...
            if  (proposal.VotingStartBlock < 0) OR  
                ...
                // sender bonded gives to ValidatorPubKey after start of vote OR if
                // sender unbonded gives from ValidatorPubKey after start of vote OR if
              ...
              if (sender is not equal to GovPubKey that corresponds to txGovVote.ValidatorPubKey)
                ...
                if sender does not have bonded gives to txGovVote.ValidatorPubKey then
                  throw
                  ...
                  if txGovVote.ValidatorPubKey is not in votersList under txGovVote.ValidatorPubKey then
                    ...
                    if exists(MinusesList[txGovVote.ValidatorPubKey]) then
                      ...
                      // by the amount of gives sender has bonded to ValidatorPubKey
                      ...
                      // at the amount of gives sender has bonded to ValidatorPubKey
