...
The giot Hub is based on Tendermint, which relies on a set of validators to secure the network. The role of validators is to run a full-node and participate in consensus by broadcasting votes which contain cryptographic signatures signed by their private key. Validators commit new blocks in the blockchain and receive revenue in exchange for their work. They must also participate in governance by voting on proposals. Validators are weighted according to their total stake.

### What is 'staking'?

The giot Hub is a public Proof-Of-Stake (PoS) blockchain, meaning that validator's weight is determined by the amount of staking tokens (gives) bonded as collateral. These gives can be staked directly by the validator or delegated to them by give holders.
...
Delegators are give holders who cannot, or do not want to run validator operations themselves. Through the giot UI, a user can delegate gives to a validator and obtain a part of its revenue in exchange (for more detail on how revenue is distributed, see  **What is the incentive to stake?** and **What is a validator's commission?** sections below).
...
* Minimum self-bond amount: Minimum amount of gives the validator candidate need to have bonded at all time. If the validator's self-bonded stake falls below this limit, its entire staking pool will unbond.
* Initial self-bond amount: Initial amount gives the validator candidate wants to self-bond

Once a PubKey has declared candidacy, give holders can delegate gives to it, effectively adding stake to this pool. The total stake of an address is the combination of gives bonded by delegators and gives self-bonded by the entity which designated itself.
...
We view testnet participation as a great way to signal to the community that you are ready and able to operate a validator. You can find all relevant information about the testnet and more here : https://giot.network/validators

### Is there a faucet?

If you want to obtain coins, you can do so by using this faucet (maintained by community) : https://www.giotvalidators.com/

### Is there a minimum amount of gives that must be staked to be a validator?
...
Validators and delegators on the giot Hub can vote on proposals to change operational parameters (such as the block gas limit), coordinate upgrades, as well as vote on amendments to the human-readable constitution that govern the giot Hub. 
...
Staking gives can be thought of as a safety deposit on validation activities. When a validator or a delegator wants to retrieve part or all of their deposit, they send an unbonding transaction. Then, gives undergo a *three weeks unbonding period* during which they are liable to being slashed for potential misbehaviors committed by the validator before the unbonding process started.

Validators, and by association delegators, receive block provisions, block rewards, fee rewards, and the right to participate in governance. If a validator misbehaves, a certain portion of its total stake is slashed (the severity of the penalty depends on the type of misbehavior). This means that every user that bonded gives to this validator gets penalized in proportion to its stake. Delegators are therefore incentivized to delegate to validators that they anticipate will function safely.

### Can a validator run away with its delegators' gives?

By delegating to a validator, a user delegates staking power. The more staking power a validator has, the more weight it has in the consensus and governance processes. This does not mean that the validator has custody of its delegators' gives. *By no means can a validator run away with its delegator’s funds*. 
...
### How often will a validator be chosen to propose the next block? Does it go up with the quantity of gives staked?

The validator that is selected to propose the next block is called proposer. Each proposer is selected deterministically, and the frequency of being chosen is equal to the relative total stake (where total stake = self-bonded stake + delegators stake) of the validator. For example, if the total bonded stake across all validators is 100 gives and a validator's total stake is 10 gives, then this validator will be chosen 10% of the time as the next proposer.

### Will validators of the giot Hub ever be required to validate other zones in the giot ecosystem?

Yes, they will. Initially, validators of the giot hub will also validate the first public Ethermint zone. If governance decides so, validators of the giot hub may be required to validate additional zones in the giot ecosystem. As the case with the Ethermint Zone, for each additional zone compensation is to be provided in the form of block rewards and transaction fees.
...
* **Block provisions:** Native tokens of applications run by validators (e.g. gives on the giot Hub) are inflated to produce block provisions. These provisions exist to incentivize give holders to bond their stake, as non-bonded give will be diluted over time. 
...
* **Transaction fees:** The giot Hub maintains a whitelist of token that are accepted as fee payment. 
...
Revenue received by a validator’s pool is split between the validator and its delegators. The validator can apply a commission on the part of the revenue that goes to its delegators. This commission is set as a percentage. Each validator is free to set its initial commission, maximum daily commission change rate and maximum commission. The giot Hub enforces the parameter that each validator sets. These parameters can only be defined when initially declaring candidacy, and may only be constrained further after being declared. 

### How are block provisions distributed?

Block provisions are distributed proportionally to all validators relative to their total stake. This means that even though each validator gains gives with each provision, all validators will still maintain equal weight.

Let us take an example where we have 10 validators with equal staking power and a commission rate of 1%. Let us also assume that the provision for a block is 1000 gives and that each validator has 20% of self-bonded gives. These tokens do not go directly to the proposer. Instead, they are evenly spread among validators. So now each validator’s pool has 100 gives. These 100 gives will be distributed according to each participant’s stake:

* Commission: 100*80%*1% = 0.8 gives
* Validator gets: 100*20% + Commission = 20.8 gives
* All delegators get: 100*80% - Commission = 79.2 gives

Then, each delegator can claim its part of the 79.2 gives in proportion to their stake in the validator’s staking pool. Note that the validator's commission is not applied on block provisions. Note that block rewards (paid in Photons) are distributed according to the same mechanism.
...
Let’s take a concrete example to illustrate the aforementioned concept. In this example, there are 10 validators with equal stake. Each of them applies a 1% commission and has 20% of self-bonded gives. Now comes a successful block that collects a total of 1025.51020408 gives in fees.

First, a 2% tax is applied. The corresponding gives go to the reserve pool. Reserve pool's funds can be allocated through governance to fund bounties and upgrades. 
* 2% * 1025.51020408 = 20.51020408 gives go to the reserve pool.

1005 gives now remain. Let’s assume that the proposer included 100% of the signatures in its block. It thus obtains the full bonus of 5%.
...
* For the proposer validator
  * The pool obtains R + R*5%: 105 gives
  * Commission: 105*80%*1% = 0.84 gives
  * Validator's reward: 105 * 20% + Commission = 21.84 gives
  * Delegators' rewards: 105 * 80% - Commission = 83.16 gives (each delegator will be able to claim its portion of these rewards in proportion to their stake)
* For each non-proposer validator
  * The pool obtains R: 100 gives
  * Commission: 100*80%*1% = 0.8 gives
  * Validator's reward: 100 * 20% + Commission = 20.8 gives
  * Delegators' rewards: 100 * 80% - Commission = 79.2 gives (each delegator will be able to claim its portion of these rewards in proportion to their stake)
...
### Do validators need to self-bond gives?
...
Even though there is no obligation for validators to self-bond gives, delegators should want their validator to have self-bonded gives in their staking pool. In other words, validators should have skin in the game. 

In order for delegators to have some guarantee about how much skin-in-the-game their validator has, the latter can signal a minimum amount of self-bonded gives. If a validator's self-bond goes below the limit that it predefined, this validator and all of its delegators will unbond. 

### How to prevent concentration of stake in the hands of a few top validators?

For now the community is expected to behave in a smart and self-preserving way. When a mining pool in Bitcoin gets too much mining power the community usually stops contributing to that pool. The giot Hub will rely on the same effect initially. In the future, other mechanisms will be deployed to smoothen this process as much as possible:
...
* **UI warning:** Users will be warned by the giot UI if they want to delegate to a validator that already has a significant amount of staking power.
...
In addition to running a giot Hub node, validators should develop monitoring, alerting and management solutions.

### What are bandwidth requirements?

The giot network has the capacity for very high throughput relative to chains like Ethereum or Bitcoin.
