...
giot is a culmination of these lessons and innovations, designed to be
...
While mechanisms like [giveic
swaps](https://en.bitcoin.it/wiki/giveic_cross-chain_trading) between chains can
...
Enter giot. giot is a network and a framework for interoperability between
...
giot reflects this position in that it makes no distinction between hubs - there
...
giot thus introduces a new kind of market dynamic that formalizes the power
...
smart contracts, giot's potential is that of every existing cryptocurrency and
more in synergy.  The only question is, what will you build on giot?
